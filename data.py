...
class SegDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        img = Image.open(img)
        pmap = io.imread(pmap, True)
        pmap = Image.fromarray(pmap)
...
class ToTensor(object):
    ...
    def __call__(self, sample):
        ...
        return {'image': transforms.functional.to_tensor(img), 'pmap': transforms.functional.to_tensor(pmap)}
...
def generate_csv(path):
    img_path = os.path.join(path, 'images','images')
    lb_path = os.path.join(path, 'labels','labels')
    cwd = os.getcwd()
    csv_file_path = os.path.join(cwd, 'data', 'image_files_pilot.csv')
    file_list_img = [name for name in os.listdir(img_path) if 
                      os.path.isfile(os.path.join(img_path, name))]
    file_list_img.sort()
    file_list_lb = [name for name in os.listdir(lb_path) if 
                      os.path.isfile(os.path.join(lb_path, name))]
    file_list_lb.sort()

    num_of_img = len(file_list_lb)
    ...
    with open(csv_file_path, 'w', newline='') as csvfile:
        ...
        for i in range(0, num_of_img):
            ...
            lb = os.path.join(lb_path, str(file_list_lb[i]))
            filewriter.writerow([img, lb])
